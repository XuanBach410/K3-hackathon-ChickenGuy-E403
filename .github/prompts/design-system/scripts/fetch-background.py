...
def get_background_image(slide_type: str) -> dict:
    ...
    if slide_config:
        keywords = slide_config.get(
            "search_keywords", slide_config.get("image_category", slide_type)
        )
...
def generate_css_for_background(
    result: dict, slide_class: str = ".slide-with-bg"
) -> str:
...
def main():
    ...
    parser.add_argument(
        "slide_type", nargs="?", help="Slide type (hero, vision, team, etc.)"
    )
    parser.add_argument(
        "--list", action="store_true", help="List available slide types"
    )
    parser.add_argument(
        "--css", action="store_true", help="Output CSS for the background"
    )
