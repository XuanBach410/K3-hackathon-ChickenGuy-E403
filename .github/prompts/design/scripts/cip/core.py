...
def get_cip_brief(brand_name, industry_query, style_query=None):
    ...
    industry = (
        industry_results.get("results", [{}])[0]
        if industry_results.get("results")
        else {}
    )
    ...
    style = (
        style_results.get("results", [{}])[0] if style_results.get("results") else {}
    )
    ...
    return {
        ...
        "color_system": {
            ...
            "secondary": style.get(
                "Secondary Colors", industry.get("Secondary Colors", "")
            ),
        },
