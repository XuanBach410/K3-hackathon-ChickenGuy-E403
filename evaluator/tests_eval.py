...
class EvalQuestionsIntegrationTests(TestCase):
    def setUp(self):
        with open(os.path.join(BASE_DIR, 'eval', 'test_questions.json'), 'r', encoding='utf-8') as f:
    ...
    def test_missing_topic_code_has_structured_error(self):
        response = self.client.post(
            '/api/evaluate/final/',
            data=json.dumps({'team_members': [{'skills': 'Python:3'}], 'quiz_answers': {}}),
    ...
    def test_advisor_chat_topic_mentioned_in_text_overrides_stale_context(self):
        response = self.client.post(
            ...
            data=json.dumps({
                ...
                'topic_code': 'EDU-05',
                'team_members': [{'skills': 'Python:3'}],
                'history': [{'role': 'assistant', 'content': 'Đang phân tích EDU-05'}],
            }),
            content_type='application/json',
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_advisor_anti_hallucination_cases(self):
        cases = [
            ('Đề tài EDU-01 có bắt buộc phải dùng LangChain không?', 'EDU-01', 'Không đủ căn cứ để khẳng định. Mô tả đề tài chỉ nêu LangGraph và các công nghệ gợi ý, không quy định bắt buộc dùng LangChain.'),
            ('Đề tài EDU-04 yêu cầu triển khai trên AWS đúng không?', 'EDU-04', 'Không. Mô tả không quy định phải triển khai trên AWS mà chỉ nêu triển khai Docker + Cloud.'),
            ('EDU-03 yêu cầu camera để nhận diện hành động sinh viên đúng không?', 'EDU-03', 'Không đủ thông tin. Mô tả chỉ đề cập ghi chú quan sát, transcript và checklist, không bắt buộc dùng camera.'),
            ('HomeMatch AI Agent yêu cầu PostgreSQL phiên bản 17 phải không?', None, 'Không đủ căn cứ. Mô tả đề tài không quy định phiên bản PostgreSQL.'),
            ('ContractGuard AI Agent phải chạy trên GPU RTX 4090 đúng không?', None, 'Không đủ thông tin. Mô tả đề tài không đề cập yêu cầu phần cứng.'),
        ]
    ...
    def test_advisor_asks_for_missing_context(self):
        ...
        for message, topic_code in cases:
            with self.subTest(message=message):
                response = self.client.post(
                    '/api/advisor/chat/',
                    data=json.dumps({'message': message, 'topic_code': topic_code, 'history': []}),
                    content_type='application/json',
                )
                self.assertEqual(response.status_code, 200)
                body = response.json()
    ...
    def test_team_can_request_recommendations_without_preselecting_topic(self):
        response = self.client.post(
            ...
            data=json.dumps({
                ...
                'team_members': [{'skills': 'Python:3, React:3', 'hours_per_week': 20}],
                'history': [],
            }),
            content_type='application/json',
        )
        ...
        self.assertTrue(all(item['code'] and item['finalScore'] is not None for item in body['recommendations']))
