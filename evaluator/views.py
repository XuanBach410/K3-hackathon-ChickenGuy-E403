...
def risk_level_from_matrix(risk_matrix):
    levels = [
        risk_matrix.get(key, "Low")
        for key in ["skill_risk", "time_risk", "team_risk", "domain_risk"]
    ]
    if "High" in levels:
        return "High"
    if "Medium" in levels:
...
def extract_requested_technology(message):
    ...
    return next(
        (
            label
            for keyword, label in technology_aliases.items()
...
def grounded_technology_reply(topic, requested_technology):
    source_text = " ".join(
        str(topic.get(field, ""))
        for field in ("tech_stack", "requirements", "description")
    )
    ...
    if normalized_tech == "aws":
        if "aws" not in normalized_source and (
            "cloud" in normalized_source or "docker" in normalized_source
        ):
    ...
    if (
        normalized_tech in {"gpu rtx 4090", "gpu"}
        and "gpu" not in normalized_source
        and "rtx" not in normalized_source
    ):
        return "Không đủ thông tin. Mô tả đề tài không đề cập yêu cầu phần cứng."
    if (
        normalized_tech == "postgresql phiên bản 17"
        and "postgresql 17" not in normalized_source
        and "postgresql phiên bản 17" not in normalized_source
    ):
    ...
    if "train model" in normalized_tech:
        ...
        return (
            "Không đủ thông tin. Mô tả đề tài không yêu cầu train model from scratch."
        )
...
def advisor_chat(request):
    ...
    try:
        ...
        topic_code_from_body = body.get("topic_code")
        topic_code = None
        if True:
            ...
            for t in all_topics:
                ...
                if (
                    re.search(r"\b" + re.escape(code) + r"\b", message, re.IGNORECASE)
                    or title
                    and len(title) > 3
                    and title.lower() in message.lower()
                ):
                    topic_code = code
                    break
                ...
                if re.search(
                    r"\b" + re.escape(code_no_dash) + r"\b", message, re.IGNORECASE
                ):
                    topic_code = code
                    break

        if not topic_code:
            topic_code = topic_code_from_body
        ...
        if moderation.action != ContentModerationGate.ACTION_ALLOW:
            return JsonResponse(
                {
                    "reply": moderation.safe_reply
                    or "Yêu cầu cần thêm ngữ cảnh trước khi chatbot có thể hỗ trợ.",
                    "suggested_questions": [
                        "EDU-01 có skill gap nào?",
                        "Nhóm nên chọn đề tài nào với kỹ năng hiện tại?",
                    ],
                    "context_window_size": len(context),
                    "context_limit": 10,
                    "resolved_topic_code": None,
        ...
        mcda = (
            calculate_mcda_score(team_members, topic)
            if topic and team_members
            else None
        )
        outcome_analysis = (
            AgentToolRegistry.execute("analyze_topic_outcomes", {"topic": topic})
            if topic
            else None
        )
        ...
        constraints = (
            outcome_analysis.get("constraints", []) if outcome_analysis else []
        )
        ...
        if outcomes:
            suggested_questions.append(
                f"Nhóm sẽ chứng minh outcome '{outcomes[0]}' bằng artifact nào ở buổi demo?"
            )
        if critical:
            skill = critical[0]
            suggested_questions.append(
                f"Ai phụ trách bù kỹ năng {skill['tech']} trong khoảng {skill.get('cost_hours', 20)} giờ?"
            )
        if kpis:
            suggested_questions.append(
                f"Nhóm sẽ đo KPI '{kpis[0]}' trên tập kiểm thử nào?"
            )
        ...
        policy_reply = (
            grounded_technology_reply(topic, requested_technology)
        ...
        requested_without_topic = (
            extract_requested_technology(message) if not topic else None
        )

        if api_key and not policy_reply:

            schemas_str = json.dumps(
                AgentToolRegistry.get_tool_schemas(), ensure_ascii=False
            )
            system_prompt = f"""
...
5. Luôn chủ động, phân tích kỹ và tư vấn sâu sắc. Trả lời bằng tiếng Việt.
            ...
            llm_result = LLMProvider.call_llm(
                provider, api_key, system_prompt, user_prompt
            )

        is_feasibility_question = any(
        ...
        if policy_reply:
            ...
            if fit_state == "PERFECT_FIT":
                reply = (
                    f"Đánh giá đề tài {topic.get('code')} phù hợp với nhóm (điểm MCDA {score}%). "
                    + (
                        "; ".join(mcda.get("explanation", []))
                        or "Kỹ năng và thời gian đáp ứng tốt."
                    )
                )
            elif fit_state == "ABLE_TO_LEARN":
                reply = (
                    f"Đánh giá đề tài {topic.get('code')} khả thi nếu nhóm chủ động học bù skill gap. "
                    + f"Dự kiến tốn ~{risk.get('total_learning_hours', 20)} giờ tự học cho các kỹ năng thiếu."
                )
            else:
            ...
            keyword in lower_message for keyword in ["outcome", "đầu ra", "sản phẩm"]
        ):
            reply = "Outcome bắt buộc từ dữ liệu đề tài gồm: " + (
                "; ".join(outcomes[:3]) or "chưa có outcome đủ rõ trong dữ liệu nguồn."
            )
        elif any(keyword in lower_message for keyword in ["kỹ năng", "skill", "thiếu"]):
            if missing:
                reply = "Skill gap hiện tại: " + "; ".join(
                    f"{item['tech']} ({item['criticality']}, ~{item.get('cost_hours', 20)}h)"
...
def evaluate_final(request):
    ...
    try:
        ...
        missing_formatted = [
            f"{m['tech']} (Mức độ: {m.get('criticality')}, {m.get('cost_hours')} giờ)"
            if isinstance(m, dict)
            else m
            for m in base_mcda.get("missingTechs", [])
        ]
        ...
        - Kỹ năng nhóm hiện có: {[str(m.get("skills", "")) for m in team_members]}
        ...
        if not llm_res:
            hours = (
                int(quiz_answers.get("3", 3))
                if isinstance(quiz_answers.get("3"), (int, str))
                else 3
            )
            ...
            llm_res = {
                "fitState": fit_state,
                "verdictTitle": base_mcda["verdictLabel"]
                ...
                "feasibilityIndex": 4.5
                if fit_state == "PERFECT_FIT"
                else 3.2
                if fit_state == "ABLE_TO_LEARN"
                else 1.8,
                "riskLevel": "Low"
                if fit_state == "PERFECT_FIT"
                else "Medium"
                ...
                "weeklyRoadmap": [
                    {
                        "week": 1,
                        "title": "Tuần 1: Khởi tạo & Nghiên cứu",
                        "tasks": [
                            "Thiết lập môi trường",
                            "Học bổ sung kỹ năng thiếu: "
                            + ", ".join(
                                [
                                    m["tech"] if isinstance(m, dict) else m
                                    for m in base_mcda["missingTechs"][:2]
                                ]
                            ),
                        ],
                    },
                    {
                        "week": 2,
                        "title": "Tuần 2: Xây dựng Prototype MVP",
                        "tasks": [
                            "Xây dựng các API cốt lõi",
                            "Tích hợp dữ liệu thử nghiệm",
                        ],
                    },
                    {
                        "week": 3,
                        "title": "Tuần 3: Hoàn thiện Tính năng",
                        "tasks": [
                            "Ghép nối Frontend + Backend",
                            "Đánh giá nội bộ lần 1",
                        ],
                    },
                    {
                        "week": 4,
                        "title": "Tuần 4: Kiểm thử & Tối ưu",
                        "tasks": [
                            "Chạy test cases với 3+ user thật",
                            "Sửa lỗi và tối ưu hiệu năng",
                        ],
                    },
                    {
                        "week": 5,
                        "title": "Tuần 5: Tài liệu & Nâng cao",
                        "tasks": [
                            "Viết báo cáo spec.md",
                            "Chuẩn bị slide demo 6 trang",
                        ],
                    },
                    {
                        "week": 6,
                        "title": "Tuần 6: Demo & Bảo vệ",
                        "tasks": ["Chạy thử dry run", "Bảo vệ đồ án tại CP6"],
                    },
                ],
                "requiredSkillSummary": {
                    "covered": [m["tech"] for m in base_mcda["matchedTechs"]],
                    "toLearn": [
                        f"{m['tech']} (~{m.get('cost_hours', 20)} giờ)"
                        if isinstance(m, dict)
                        else f"{m} (~20 giờ)"
                        for m in base_mcda["missingTechs"]
                    ],
                },
            }

        llm_res = ensure_evidence_contract(
            llm_res, base_mcda, source=provider if api_key else "rule-based"
        )
        return JsonResponse(
            {"topic_code": topic_code, "evaluation": llm_res, "base_mcda": base_mcda}
        )
