...
def payload_for_case(case):
    ...
    missing_context = "Thiếu thông tin nhóm" in category or any(
        ...
        for phrase in ("team e 2 ng", "đề tài này có phù hợp", "đề tài ai nào dễ nhất", "easy hay hard")
    )
...
def rubric(case, response):
    ...
    if "langchain" in message:
        return contains_all(reply, ["không đủ căn cứ", "langgraph", "không quy định"]), "anti-hallucination: LangChain"
    if "aws" in message:
        return contains_all(reply, ["không", "aws", "docker", "cloud"]), "anti-hallucination: AWS"
    if "camera" in message:
        return contains_all(reply, ["không đủ thông tin", "camera"]), "anti-hallucination: camera"
    if "postgresql phiên bản 17" in message:
        return contains_all(reply, ["không đủ căn cứ", "phiên bản postgresql"]), "anti-hallucination: version"
    if "gpu" in message:
        return contains_all(reply, ["không đủ thông tin", "phần cứng"]), "anti-hallucination: hardware"
    if "docker" in message and "edu04" in message:
        return "docker" in reply and ("không đủ" in reply or "không" in reply), "anti-hallucination: Docker"
    ...
    if any(term in message for term in ("source code", "báo cáo", "đáp án", "slide bảo vệ", "video demo", "thi hộ")):
        return moderation == "OUT_OF_SCOPE" and "từ chối" in reply, "moderation: out of scope"

    # Ambiguity must be stopped or turned into a concrete request for missing evidence.
    if any(term in message for term in ("đề tài này có phù hợp", "team e 2 ng", "easy hay hard", "đề tài ai nào dễ nhất")):
        return moderation == "NEEDS_CONTEXT" and any(term in reply for term in ("chưa đủ", "cần", "tên", "mã đề tài")), "moderation: missing context"

    if "nên chọn đề tài nào" in message and not response.get("recommendations"):
        return moderation == "NEEDS_CONTEXT", "moderation: recommendation needs evidence"
    ...
    if any(term in message for term in ("mvp", "fleet miner", "simfeedback", "homematch", "1 tháng", "2 tuần", "4 tháng")):
        evidence = response.get("mcda_snapshot") or {}
        has_reasoning = any(term in reply for term in ("mcda", "rủi ro", "skill", "phạm vi", "mvp", "năng lực", "thời gian"))
    ...
    expected_terms = [term for term in re.findall(r"[a-zA-ZÀ-ỹ0-9]{4,}", expected) if term not in {"phải", "được", "với", "nhóm", "đề", "tài"}]
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        fieldnames = [
            "id", "category", "input", "expected_response", "actual_response",
            "moderation_action", "decision_source", "status_code", "rubric", "passed",
        ]
